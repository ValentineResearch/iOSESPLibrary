...
/// @return YES if the packet was sent, or NO if the packet was too large or if the client was not connected
-(BOOL)sendPacket:(ESPPacket*)packet;
/// Send data to the ESP device.
/// @param data the data to send
/// @return YES if the data was sent, or NO if the data was too large or if the client was not connected
...
/// Requests the user bytes data of the ESP device
...
/// Requests the user bytes data of the Valentine One
/// @param completion a callback called if a response is received, if the request times out, or if an error occurs
...
/// Requests the current volume settings of the ESP device
/// @param target the target to send the request to. Currently only ESPRequestTargetValentineOne is able to handle this type of request
/// @param completion a callback called if a response is received, if the request times out, or if an error occurs
-(void)requestCurrentVolumeFrom:(ESPRequestTarget)target completion:(void (^)(NSData *, NSError *))completion;
/// Requests the current volume settings of the Valentine One
/// @param completion a callback called if a response is received, if the request times out, or if an error occurs
-(void)requestCurrentVolume:(void (^)(NSData *, NSError *))completion;
/// Writes the given volume settings bytes to the ESP device
