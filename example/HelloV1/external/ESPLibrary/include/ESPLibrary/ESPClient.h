...
//! Handles requests and responses to and from an ESP device
@interface ESPClient : NSObject <CBPeripheralDelegate>

/*! Sends an ESPPacket to the ESP device.
...
/*! Sends data to the ESP device.
	@param data the data to send
	@returns YES if the data was sent, or NO if the data was too large or if the client was not connected */
-(BOOL)sendData:(NSData*)data;

/*! Called whenever a valid ESPPacket is received. This method forwards received packets to their delegate methods, so the super method should be called when overriding.
	@param packet the received packet */
-(void)didReceivePacket:(ESPPacket*)packet;
/*! Called whenever an ESPPacket with a bad checksum is received
	@param packet the received packet */
...
/*! Requests the ESP device or one of its accessories to set itself to its factory default settings
	@param target the target to send the request to
	@param completion a callback called if the request is successful, if the request times out, or if an error occurs */
...
/*! Writes all the custom sweeps to the ESP device
	@param sweeps the sweeps (frequency ranges) to write to the ESP device
	@param target the target to send the request to
	@param completion a callback called if the request is successful, if the request times out, or if an error occurs */
...
/*! Requests the current savvy status from the ESP device's savvy accessory
...
/*! Requests the current vehicle speed from the ESP device's savvy accessory
	@param target the target to send the request to. Currently only ESPRequestTargetSavvy is able to handle this type of request
	@param completion a callback called if a response is received, if the request times out, or if an error occurs */
-(void)requestVehicleSpeedFrom:(ESPRequestTarget)target completion:(void(^)(NSUInteger vehicleSpeedKPH, NSError* error))completion;

/*! Requests the thumbwheel speed of the ESP device's savvy accessory to be overridden
...
/*! Enables or disables the unmute functionality of the ESP device's savvy accessory
	@param unmuteEnabled whether or not to unmute the ESP device's savvy
	@param target the target to send the request to
	@param completion a callback called if a response is received, if the request times out, or if an error occurs */
